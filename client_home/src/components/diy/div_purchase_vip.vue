<template>
	<view class="diy_details diy_div_purchase_vip">
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/purchase_vip/get_obj", {
					purchase_vip_id:this.query.purchase_vip_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["purchase_vip_id"] !==0) {
