<template>
  <view id="home" class="home--new">
    ...
    <Card
      class="list_diy"
      title="线上图书推荐"
      url="/pages/online_books/list"
      :list="list_article"
      v-if="$check_action('/online_books/list', 'get')"
    >
	  <list_online_books :list="list_online_books"></list_online_books>
...
<script>
				...
				import list_online_books from "@/components/diy/list_online_books.vue";
  ...
  methods: {
    ...
    toToggle() {
      this.isAdmin = !this.isAdmin;
		...
		*/
		get_online_books() {
			let url = "~/api/online_books/get_list?";
													url = "~/api/online_books/get_hits_list?";
			
			this.$get(url, {
									"user_id": this.$store.state.user.user_id,
											}, (json) => {
					if (json.result && json.result.list) {
					console.log("线上图书" ,json.result.list);
					this.list_online_books = json.result.list;
											this.list_online_books.map((o)=>{
							o["praise_len"];
						});
						this.get_praise(this.list_online_books ,"online_books" ,"6920");
									}
			});
		},
