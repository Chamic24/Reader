<template>
	<view class="page_online_books diy_detail diy_detail--new" id="online_books_details">
		<view class="warp">
			<view class="container-fluid">
				<view class="row">
					...
					<view v-if="$check_field('get','book_category')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>图书类别</span>
							</view>
					...
					<view v-if="$check_field('get','press')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>出版社</span>
							</view>
					...
					<view v-if="$check_field('get','vip_price')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>VIP价格</span>
							</view>
					...
					<view v-if="$check_field('get','book_introduction')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>图书简介</span>
							</view>
		...
		<view class="warp">
			<view class="container-fluid container-fluid-comment--new">
				<view class="row">
					<view class="col">
									<button type="button" class="btn btn-primary" v-if="$check_action('/personal_bookshelf/edit','add')" @click="to_form('/pages/personal_bookshelf/edit')">
							<span>加入书架</span>
						</button>
								<button type="button" class="btn btn-primary" v-if="$check_action('/purchase_vip/edit','add')" @click="to_form('/pages/purchase_vip/edit')">
							<span>购买VIP</span>
						</button>
...
<!--						<button type="button" class="btn btn-primary" @click="cancel()">返回</button>-->
						...
						<navigator class="btn link"
							:url="'/pages/comment/edit?source_table=online_books&source_field=online_books_id&source_id=' + obj['online_books_id']">
							评论
						</navigator>
	...
	export default {
		...
		methods: {
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					source_table: "online_books",
					source_field: "online_books_id",
					source_id: obj["online_books_id"]
				}, (res) => {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "online_books",
					source_field: "online_books_id",
					source_id: this.obj["online_books_id"],
					user_id
				};
        ...
        _this.obj.praise_len = parseInt(_this.obj.praise_len)
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/online_books/set?online_books_id=' + _this.obj["online_books_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/online_books/set?online_books_id=' + obj["online_books_id"], {
					hits
				}, (res) => {
					if(res.result){
