<template>
	<view class="page_online_reading diy_detail diy_detail--new" id="online_reading_details">
		<view class="warp">
			<view class="container-fluid">
				<view class="row">
					...
					<view v-if="$check_field('get','chapter')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>章节</span>
							</view>
					...
					<view v-if="$check_field('get','content')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>内容</span>
							</view>
								<view class="diy_field diy_html">
								<rich-text class="rich_text" :nodes="obj['content']"></rich-text>
							</view>
							</view>
					</view>
				</view>
		...
		<view class="warp">
			<view class="container-fluid container-fluid-comment--new">
				<view class="row">
					<view class="col">
									<button type="button" class="btn btn-primary" v-if="$check_action('/read_settings/edit','add')" @click="to_form('/pages/read_settings/edit')">
							<span>阅读设置</span>
						</button>
...
<!--						<button type="button" class="btn btn-primary" @click="cancel()">返回</button>-->
						...
						<navigator class="btn link"
							:url="'/pages/comment/edit?source_table=online_reading&source_field=online_reading_id&source_id=' + obj['online_reading_id']">
							评论
						</navigator>
	...
	export default {
		...
		methods: {
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					source_table: "online_reading",
					source_field: "online_reading_id",
					source_id: obj["online_reading_id"]
				}, (res) => {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "online_reading",
					source_field: "online_reading_id",
					source_id: this.obj["online_reading_id"],
					user_id
				};
        ...
        _this.obj.praise_len = parseInt(_this.obj.praise_len)
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/online_reading/set?online_reading_id=' + _this.obj["online_reading_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/online_reading/set?online_reading_id=' + obj["online_reading_id"], {
					hits
				}, (res) => {
					if(res.result){
