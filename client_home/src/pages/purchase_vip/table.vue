<template>
  <view class="page_purchase_vip diy_table diy_table_tml" id="purchase_vip_table">
    <!-- 筛选模块(开始) -->
    <view>
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/purchase_vip/view?" v-show="user_group == '管理员' || $check_action('/purchase_vip/table','add') || $check_action('/purchase_vip/view','add')">添加</navigator>
						...
						<view class="view">
							<view class="diy_title" v-if="true">
								...
								<span>
									...
									<view class="col" v-if="1 && $check_field('get','user_number')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>用户编号</span>
							</view>
				...
				<view class="col">
					<view class="view">
						<view class="create_time_block diy_filed diy_time">
                          ...
                          <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/purchase_vip/table','del') || $check_action('/purchase_vip/view','del')">删除</button>
                        </view>
					</view>
				</view>
				<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/purchase_vip/view?' + field + '=' + o[field]"
						v-show="user_group == '管理员' || $check_action('/purchase_vip/table','set') || $check_action('/purchase_vip/view','set') || $check_action('/purchase_vip/view','get')" class="btn btn-primary">详情</navigator>
											<a @click="goto_pay(o)" size="mini"  v-if="o.pay_state==='未支付' && $check_pay('/purchase_vip/table')" class='btn btn-primary'>支付</a>
					                    									</view>
			</view>
		</view>
	</view>
          ...
          submit_pay(){
            ...
            this.$post(url,param,(res)=>{
              if(res.result){
                _this.$toast("支付成功");
                setTimeout(()=>{
                  _this.$nav("/pages/purchase_vip/table");
                }, 700);
              }else if(res.error){
