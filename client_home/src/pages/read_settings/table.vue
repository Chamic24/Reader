<template>
  <view class="page_read_settings diy_table diy_table_tml" id="read_settings_table">
    <!-- 筛选模块(开始) -->
    <view>
              ...
              <uni-forms :modelValue="query">
                    <uni-forms-item label="图书名称" name="book_name">
                      <uni-easyinput type="text" v-model="query.book_name" placeholder="图书名称" />
                        ...
                        <uni-forms-item label="章节" name="chapter">
                      <uni-easyinput type="text" v-model="query.chapter" placeholder="章节" />
                    </uni-forms-item>
                            </uni-forms>
              <!-- /搜索栏 -->
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/read_settings/view?" v-show="user_group == '管理员' || $check_action('/read_settings/table','add') || $check_action('/read_settings/view','add')">添加</navigator>
						...
						<view class="view">
							<view class="diy_title" v-if="true">
								...
								<span>
									...
									<view class="col" v-if="1 && $check_field('get','chapter')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>章节</span>
							</view>
						...
						<view class="view">
							<view class="diy_title" v-if="true">
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','reading_background')">
						<view class="view">
							<view class="diy_title" v-if=" false">
								<span>阅读背景</span>
							</view>
															<view class="diy_field diy_img">
									<img :src="$fullUrl(o['reading_background'])" width="100%" height="100" />
								</view>
													</view>
					</view>
									<view class="col" v-if="0 && $check_field('get','font_size')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>字体大小</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','font_style')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>字体样式</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','reading_progress')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>阅读进度</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','read_comments')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>阅读批注</span>
							</view>
				...
				<view class="col">
					<view class="view">
						<view class="create_time_block diy_filed diy_time">
                          ...
                          <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/read_settings/table','del') || $check_action('/read_settings/view','del')">删除</button>
                        </view>
					</view>
				</view>
				<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/read_settings/view?' + field + '=' + o[field]"
						v-show="user_group == '管理员' || $check_action('/read_settings/table','set') || $check_action('/read_settings/view','set') || $check_action('/read_settings/view','get')" class="btn btn-primary">详情</navigator>
					                    									</view>
			</view>
		</view>
	</view>
		...
		data() {
			return {
                ...
                list_user_user_number: [],
                                // 字体大小列表
                list_font_size: [{value:"",text:"全部"}],
                            // 字体样式列表
                list_font_style: [{value:"",text:"全部"}],
                        			}
		},
		methods: {
          search() {
